...
A customer-level report showing 2005 performance:
    ...
    first_2005_rental, last_2005_rental

    total_2005_rentals

    total_2005_spend

    recency_score, frequency_score, monetary_score, health_score (0-100)
...
WITH recency_cte AS (
    SELECT sq1.customer_id, sq1.rental_month AS latest_rental_month,
    CASE
        WHEN sq1.rental_month BETWEEN 10 AND 12
            THEN 'Q4'
         WHEN sq1.rental_month BETWEEN 07 AND 09
            THEN 'Q3'
        ELSE
            'H1'
    ...
    FROM (
        ...
        EXTRACT(MONTH FROM MAX(r.rental_date)) AS rental_month
        FROM customer c
