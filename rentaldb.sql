...
CROSS JOIN total_films, total_rentals; --Cross Join to join single value tables.
...
FROM (
  SELECT customer_id, SUM(amount) AS total_revenue --Subquery to calculate total anount spent by each customer.
  FROM payment
...
--Comparing total amount spend by a customer with average for filtering
...
LIMIT 10; --Limiting by top 10 cities
...
SELECT 
  ...
  s.email, 
  COUNT(*) AS rental_count
FROM rental r JOIN staff s ON r.staff_id = s.staff_id --Joing Staff table to retrive identifiable staff data
...
WHERE r.return_date IS NULL; --Filtering customers whose returned date is Null - Unreturned films.
...
f.title, 
c.name,
SUM(p.amount) AS total_revenue_per_film, -- calculates total revenue generated by that film.
COUNT(r.rental_id) AS no_of_times_film_rented, --calculates Total no of times film was rented.
...
avg_metrics AS (
  ...
  AVG(no_of_times_film_rented) AS avg_no_of_times_film_rented
  --FROM film_metrics CTE calculates avg_no_of_times_film_rented - Rental frequency (no_of_times_film_rented/no.of.rental_id)
  FROM film_metrics
  )
...
WITH above_avg_rental_frequency AS (
  ...
  --Filtering customers based on average frequency
    (SELECT AVG(times_rented) FROM (SELECT r.customer_id, 
...
above_avg_spend AS (
  ...
  GROUP BY 1
  HAVING SUM(p.amount) > 
    --Filtering customers based on avg amount spent criteria
    (SELECT AVG(tot_spend) 
      FROM (SELECT SUM(amount) AS tot_spend
      FROM payment p
      GROUP BY customer_id))
),
...
WITH ab_avg_rental_rate AS (
  ...
  FROM film f
  WHERE rental_rate > (SELECT AVG(rental_price) 
...
rental_frequency AS (
  SELECT f.film_id, COUNT(f.film_id) AS no_of_times_film_rented FROM film f
  INNER JOIN inventory i ON f.film_id = i.film_id
  INNER JOIN rental r ON i.inventory_id =r.inventory_id
  GROUP BY f.film_id
  HAVING COUNT(f.film_id) < (SELECT AVG(rental_count) 
              FROM (SELECT f2.film_id, 
...
available_inventory AS (
  SELECT DISTINCT f.film_id FROM film f --Replace with * from detailed idea
INNER JOIN inventory i ON f.film_id=i.film_id
...
--Main Query
SELECT avrr.film_id,avrr.title FROM ab_avg_rental_rate AS avrr
...
WITH abv_avg_rental_f AS (
  ...
  HAVING COUNT(r.rental_id) > 
          (SELECT AVG(avg_rental_freq) 
            FROM (SELECT f2.film_id, COUNT(r2.rental_id) AS avg_rental_freq
              FROM film f2
              INNER JOIN inventory i2 ON f2.film_id = i2.film_id
              INNER JOIN rental r2 ON i2.inventory_id = r2.inventory_id
              GROUP BY f2.film_id))
