...
SELECT st.film_id,
  st.title,
  st.store_id,
  st.total_copies_store,
  st.rented,
  st.available_inventory,
  -- Calculate the percentage of available copies.
  ROUND(
    st.available_inventory::numeric / st.total_copies_store * 100,
    2
...
WHERE ROUND(
    st.available_inventory::numeric / st.total_copies_store * 100,
    2
  ) < 50 -- Filter for films with less than 50% availability.
